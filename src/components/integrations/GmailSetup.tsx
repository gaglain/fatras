...
import { Mail, ExternalLink, Check, AlertCircle } from 'lucide-react';

export const GmailSetup: React.FC = () => {
  ...
  const saveConfiguration = () => {
    ...
    localStorage.setItem('gmailConfig', JSON.stringify(configToSave));
        ...
        {config.isConfigured && (
          <div className="bg-green-50 p-3 rounded-lg">
            <p className="text-green-800 text-sm flex items-center">
              <Check className="h-4 w-4 mr-2" />
              Gmail est configuré et prêt à être utilisé
            </p>
          </div>
        )}
      </CardContent>
