...
export const TourStopCard: React.FC<TourStopCardProps> = ({
  ...
  const handlePreview = () => {
    console.log('Aperçu de l\'étape:', stop);
    toast.success(`Aperçu de l'étape: ${stop.city} - ${stop.venue}`);
  };

  const handleDownloadPDF = () => {
    console.log('Téléchargement PDF de l\'étape:', stop);
    toast.success(`Génération du PDF pour: ${stop.city} - ${stop.venue}`);
    
    // Simulation d'un téléchargement PDF
    const element = document.createElement('a');
    const file = new Blob([`Feuille de route - ${stop.city}\n\nLieu: ${stop.venue}\nDate: ${stop.date}\nHeure: ${stop.time}`], {type: 'text/plain'});
    element.href = URL.createObjectURL(file);
    element.download = `feuille-route-${stop.city}-${stop.date}.pdf`;
