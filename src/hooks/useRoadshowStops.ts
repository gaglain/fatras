...
export const useRoadshowStops = () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          event_date: stopData.event_date,
          event_time: stopData.event_time,
          check_in_time: stopData.check_in_time,
  ...
  const convertFromTourStop = (tourStop: Partial<TourStop>): Partial<RoadshowStop> => ({
    ...
    event_date: tourStop.date,
    event_time: tourStop.time,
    check_in_time: tourStop.checkInTime,
    departure_time: tourStop.departureTime,
