...
import { Plus, BookOpen, Edit, Eye, Share, Download } from 'lucide-react';
...
const sampleShowBibles: ShowBible[] = [
  {
    id: '1',
...
export const ShowBible: React.FC = () => {
  const [showBibles, setShowBibles] = useState<ShowBible[]>(sampleShowBibles);
  const [selectedBible, setSelectedBible] = useState<ShowBible | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {!selectedBible ? (
        ...
        <div className="space-y-4">
          {showBibles.map((bible) => (
            <Card key={bible.id} className="hover:shadow-lg transition-shadow cursor-pointer">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-3">
                      <BookOpen className="h-5 w-5 text-purple-600" />
                      <h3 className="text-xl font-semibold text-gray-900">{bible.title}</h3>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                      <div>
                        <p className="text-sm text-gray-500">Event</p>
                        <p className="font-medium">{bible.event}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Artist</p>
                        <p className="font-medium">{bible.artist}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Venue</p>
                        <p className="font-medium">{bible.venue}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Show Date</p>
                        <p className="font-medium">{new Date(bible.date).toLocaleDateString()}</p>
                      </div>
                    </div>
                    
                    <div className="mb-4">
                      <p className="text-sm text-gray-500 mb-2">Sections ({bible.sections.length})</p>
                    ...
                    <div className="flex items-center justify-between text-sm text-gray-500">
                      <span>Last updated: {new Date(bible.lastUpdated).toLocaleDateString()}</span>
                      <span>Collaborators: {bible.collaborators.join(', ')}</span>
                    </div>
                  </div>
        ...
        /* Show Bible Detail View */
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <Button variant="outline" onClick={() => setSelectedBible(null)}>
              ← Back to Show Bibles
            </Button>
            <div className="flex space-x-2">
          ...
          <Card>
            <CardHeader>
              <CardTitle>{selectedBible.title}</CardTitle>
              <div className="text-sm text-gray-600">
                {selectedBible.event} • {selectedBible.artist} • {selectedBible.venue}
              </div>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div className="lg:col-span-1">
                  ...
                  <div className="space-y-2">
                    {selectedBible.sections.map((section, index) => (
                      <button
                        key={index}
      ...
      {/* Create Show Bible Form Modal */}
      {showCreateForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <Card className="w-full max-w-lg mx-4">
            <CardHeader>
              <CardTitle>Create New Show Bible</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <Input placeholder="Show Bible Title" />
